...
export class MicroservicesController {
  ...
  handlePing(@Payload() data: any) {
    this.logger.log(`Tín hiệu ping_check nhận được từ Microservice bạn: ${JSON.stringify(data)}`);
    return { status: 'PONG', receiveTime: new Date(), message: 'Hello from Coop Backend' };
  }

  // Đón nhận Event (Bắn rồi cháy, không cần Return)
  @EventPattern('task_statistics_export')
  handleTaskStatsExport(@Payload() data: { groupId: string, requestedBy: string }) {
    this.logger.log(`📥 Bắt đầu quy trình Report ngầm (Event) cho Group: ${data.groupId}`);
